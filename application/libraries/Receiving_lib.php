...
class Receiving_lib
{
	...
	{
		return ($this->CI->session->userdata($this->prefix.'s_print_after_receiving') == 'true' ||
				$this->CI->session->userdata($this->prefix.'s_print_after_receiving') == '1');
	}
	
	public function set_print_after_transaction($print_after_receiving)
	{
		return $this->CI->session->set_userdata($this->prefix.'s_print_after_receiving', $print_after_receiving);
	}
	...
	public function get_amount_due()
	{
		...
		$receivings_total = $this->get_total();
		$amount_due = bcsub($receivings_total, $payment_total);
	...
	{
		if(!$this->CI->session->userdata($this->prefix.'s_supplier'))
		{
			$this->set_supplier(-1);
		}

		return $this->CI->session->userdata($this->prefix.'s_supplier');
	}

	public function set_supplier($supplier_id)
	{
		$this->CI->session->set_userdata($this->prefix.'s_supplier', $supplier_id);
	}

	public function remove_supplier()
	{
		$this->CI->session->unset_userdata($this->prefix.'s_supplier');
	}
		...
		$description = NULL, $serialnumber = NULL, $include_deleted = FALSE,$options=array())
	{
        ...
        $itemalreadyinreceiving = FALSE;        //We did not find the item yet.
		...
		if(!$itemalreadyinreceiving || $item_info->is_serialized)
		{
	...
	public function return_entire_transaction($receipt_receiving_id)
	{
		//POS #
	...
	public function copy_entire_transaction($receiving_id)
	{
		$this->empty_cart();
		$this->remove_supplier();
		...
		$this->set_supplier($this->CI->Receiving->get_supplier($receiving_id)->person_id);
		$this->set_employee($this->CI->Receiving->get_employee($receiving_id)->person_id);
	}
	
	public function copy_entire_suspended_transaction($receiving_id)
	{
		$this->empty_cart();
		$this->remove_supplier();

		foreach($this->CI->Receiving_suspended->get_transaction_items($receiving_id)->result() as $row)
		{
		...
		foreach($this->CI->Receiving_suspended->get_transaction_payments($receiving_id)->result() as $row)
		{
			$this->add_payment($row->payment_type, $row->payment_amount);
		}
		$suspended_receiving_info = $this->CI->Receiving_suspended->get_info($receiving_id)->row();
		$this->set_supplier($suspended_receiving_info->person_id);
		$this->set_comment($suspended_receiving_info->comment);
		$this->set_invoice_number($suspended_receiving_info->invoice_number);

		$this->set_mailState($suspended_receiving_info->mail_state);
		$this->set_transaction_id($receiving_id);
		$this->set_transaction_items_location($suspended_receiving_info->receiving_items_location);
	...
	public function is_supplier_taxable()
	{
		return FALSE;
		/*$supplier_id = $this->get_supplier();
		$supplier = $this->CI->Supplier->get_info($supplier_id);
		
		//Do not charge receivings tax if we have a supplier that is not taxable
	...
	public function apply_supplier_discount($discount_percent)
	{	
		// Get all items in the cart so far...
		$items = $this->get_cart();
