...
class Manufacture_lib
{
	protected $CI;
	public $prefix='manufacture';
	...
	{
		return ($this->CI->session->userdata($this->prefix.'s_print_after_manufacture') == 'true' ||
				$this->CI->session->userdata($this->prefix.'s_print_after_manufacture') == '1');
	}
	
	public function set_print_after_transaction($print_after_manufacture)
	{
		return $this->CI->session->set_userdata($this->prefix.'s_print_after_manufacture', $print_after_manufacture);
	}
	...
	public function get_amount_due()
	{
		...
		$manufactures_total = $this->get_total();
		$amount_due = bcsub($manufactures_total, $payment_total);
	...
	{
		if(!$this->CI->session->userdata($this->prefix.'s_staff'))
		{
			$this->set_staff(-1);
		}

		return $this->CI->session->userdata($this->prefix.'s_staff');
	}

	public function set_staff($staff_id)
	{
		$this->CI->session->set_userdata($this->prefix.'s_staff', $staff_id);
	}

	public function remove_staff()
	{
		$this->CI->session->unset_userdata($this->prefix.'s_staff');
	}
		...
		$description = NULL, $serialnumber = NULL, $include_deleted = FALSE,$options=array())
	{
        ...
        $itemalreadyinmanufacture = FALSE;        //We did not find the item yet.
		...
		$price = $price != NULL ? $price : $item_info->wage_price;
		$total = $this->get_item_total($quantity, $price, $discount);
        $discounted_total = $this->get_item_total($quantity, $price, $discount, TRUE);
		//Item already exists and is not serialized, add to quantity
		if(!$itemalreadyinmanufacture || $item_info->is_serialized)
		{
	...
	public function return_entire_transaction($receipt_manufacture_id)
	{
		//POS #
		$pieces = explode(' ', $receipt_manufacture_id);
		$manufacture_id = $pieces[1];

		$this->empty_cart();
		$this->remove_staff();

		foreach($this->CI->Manufacture->get_manufacture_items($manufacture_id)->result() as $row)
		{
		...
		$this->set_staff($this->CI->Manufacture->get_staff($manufacture_id)->person_id);
	}
	...
	public function copy_entire_transaction($manufacture_id)
	{
		$this->empty_cart();
		$this->remove_staff();
		...
		foreach($this->CI->Manufacture->get_manufacture_payments($manufacture_id)->result() as $row)
		{
			$this->add_payment($row->payment_type, $row->payment_amount);
		}

		$this->set_staff($this->CI->Manufacture->get_staff($manufacture_id)->person_id);
		$this->set_employee($this->CI->Manufacture->get_employee($manufacture_id)->person_id);
	}
	
	public function copy_entire_suspended_transaction($manufacture_id)
	{
		$this->empty_cart();
		$this->remove_staff();

		foreach($this->CI->Manufacture_suspended->get_transaction_items($manufacture_id)->result() as $row)
		{
		...
		foreach($this->CI->Manufacture_suspended->get_transaction_payments($manufacture_id)->result() as $row)
		{
			$this->add_payment($row->payment_type, $row->payment_amount);
		}
		$suspended_manufacture_info = $this->CI->Manufacture_suspended->get_info($manufacture_id)->row();
		$this->set_staff($suspended_manufacture_info->person_id);
		$this->set_comment($suspended_manufacture_info->comment);
		$this->set_invoice_number($suspended_manufacture_info->invoice_number);

		$this->set_mailState($suspended_manufacture_info->mail_state);
		$this->set_transaction_id($manufacture_id);
		$this->set_transaction_items_location($suspended_manufacture_info->manufacture_items_location);
	...
	public function is_staff_taxable()
	{
		return FALSE;
		/*$staff_id = $this->get_staff();
		$staff = $this->CI->Staff->get_info($staff_id);
		
		//Do not charge manufactures tax if we have a staff that is not taxable
		return $staff->taxable or $staff_id == -1;*/
	}
	...
	public function apply_staff_discount($discount_percent)
	{	
		// Get all items in the cart so far...
		$items = $this->get_cart();
