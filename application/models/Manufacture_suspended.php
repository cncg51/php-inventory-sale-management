<?php
class Manufacture_suspended extends CI_Model
{
	public $dbprefix='manufacture';//_suspended
	public $partner='staff';
	public $inventory_ref_type=2;
	...
	{
		$this->db->from($this->dbprefix.'s');
		$this->db->where($this->dbprefix.'_id', $manufacture_id);
	...
	*/
	...
	public function update($manufacture_data, $manufacture_id)
	{
		$this->db->where($this->dbprefix.'_id', $manufacture_id);

		return $this->db->update($this->dbprefix.'s', $manufacture_data);
	}
	
	public function save($items, $staff_id, $employee_id, $comment, 
		$invoice_number, $payments, $infos,$manufacture_id = FALSE)
	{
		if(count($items) == 0)
		{
			return -1;
		...
		$manufactures_data = array(
			...
			$this->partner.'_id'    => $this->Staff->exists($staff_id) ? $staff_id : null,
			'employee_id'    => $employee_id,
			'comment'        => $comment,
			'invoice_number' => $invoice_number,
			'mail_state'	 => $infos['mail_state'],
			'manufacture_items_location'=>$infos['manufacture_items_location']
		);

		$this->delete_together($manufacture_id);
		...
		if(empty($manufacture_id)||FALSE==$manufacture_id){
			$manufactures_data[$this->dbprefix.'_time']=$date_now;

			$this->db->insert($this->dbprefix.'s', $manufactures_data);
			$manufacture_id = $this->db->insert_id();
		}else{
			//hook_2_cart_diff
			$this->update($manufactures_data,$manufacture_id);
		}

		foreach($payments as $payment_id=>$payment)
		{
			$manufactures_payments_data = array(
				$this->dbprefix.'_id'        => $manufacture_id,
				'payment_type'   => $payment['payment_type'],
				'payment_amount' => $payment['payment_amount']
			);

			$this->db->insert($this->dbprefix.'s'.'_payments', $manufactures_payments_data);
		}
		...
		foreach($items as $line=>$item)
		{
			...
			$this->db->insert($this->dbprefix.'s'.'_items', $manufactures_items_data);
		...
		if(array_key_exists('cart_diff', $infos)){
			foreach ($infos['cart_diff'] as $item_id => $locations) {
				foreach ($locations as $item_loc => $quantities) {
					...
					if($quantity_old!=0){
						$this->db->delete('inventory',array('ref_type'=>$this->inventory_ref_type,'ref_id'=>$manufacture_id,
	...
	public function delete($manufacture_id)
	{
		...
		$this->db->delete($this->dbprefix.'s'.'_payments', array($this->dbprefix.'_id' => $manufacture_id)); 
		$this->db->delete($this->dbprefix.'s'.'_items_taxes', array($this->dbprefix.'_id' => $manufacture_id)); 
		$this->db->delete($this->dbprefix.'s'.'_items', array($this->dbprefix.'_id' => $manufacture_id)); 
		$this->db->delete($this->dbprefix.'s', array($this->dbprefix.'_id' => $manufacture_id)); 
		
		$this->db->trans_complete();
				
		return $this->db->trans_status();
	}
	public function delete_together($manufacture_id)
	{
		if(empty($manufacture_id)||FALSE==$manufacture_id){return FALSE;}
	...
	public function get_transaction_items($manufacture_id)
	{
	...
	public function get_transaction_payments($manufacture_id)
	{
	...
	public function get_comment($manufacture_id)
	{
		$this->db->from($this->dbprefix.'s');
		$this->db->where($this->dbprefix.'_id', $manufacture_id);
