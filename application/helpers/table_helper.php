...
function get_item_data_row($item, $controller)
{
	...
	return array (
		...
		'name' => anchor($controller_name."/count_details/$item->item_id",$item->name,array('class' => 'modal-dlg','title' => $CI->lang->line($controller_name.'_details_count'))),
		//'category' => $item->category,
		//'company_name' => $item->company_name,
		//'cost_price' => to_currency($item->cost_price),
		'unit_price' => to_currency($item->unit_price),
		'quantity' => anchor($controller_name."/inventory/$item->item_id", ($item->is_infinite==1)?'-':to_quantity_decimals($item->quantity),
